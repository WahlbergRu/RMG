...
namespace VoronoiMapGen.Features
{
    public static class EntityCreationPipeline
    {
        ...
        )
        {
            ...
            {
                var e = createdEntities[i];
                var cell = data.Cells[i];
                var meta = data.Meta[i];

                // Связь с родителем
                if (parentIndexToEntity.IsCreated && parentIndexToEntity.TryGetValue(meta.ParentIndex, out var pEnt))
