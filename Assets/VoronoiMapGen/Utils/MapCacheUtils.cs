...
namespace VoronoiMapGen.Utils
{
    public static class MapCacheUtils
    {
        ...
        {
            sites = default; meta = default; tectonic = default; 
            climate = default; hydro = default; biomes = default;
            geomVerts = default; geomCounts = default; geomEdges = default;
            ...
            try
            {
                ...
                using (var reader = new BinaryReader(stream))
                {
                    ...
                    for (int i = 0; i < count; i++) { tectonic[i] = new TectonicPlateData { IsOcean = reader.ReadBoolean(), BaseHeight = reader.ReadSingle() }; }
                    for (int i = 0; i < count; i++) { climate[i] = new ClimateData { Temperature = reader.ReadSingle(), Moisture = reader.ReadSingle() }; }
                    for (int i = 0; i < count; i++)
                    {
                    ...
                    for(int i=0; i<vertCount; i++) geomVerts[i] = new float2(reader.ReadSingle(), reader.ReadSingle());
                    ...
                    for(int i=0; i<edgesCount; i++)
                    {
                        ...
                        geomEdges[i] = new VoronoiEdge
                        {
                            ...
                            Level = level,
                            CellA = Unity.Entities.Entity.Null,
                            CellB = Unity.Entities.Entity.Null
                        };
                    }
                }
                return true;
            ...
            NativeList<VoronoiEdge> geomEdges)
        {
            ...
            using (var writer = new BinaryWriter(stream))
            {
                ...
                for (int i = 0; i < sites.Length; i++) { writer.Write(sites[i].x); writer.Write(sites[i].y); }
                for (int i = 0; i < meta.Length; i++) { writer.Write(meta[i].Index); writer.Write(meta[i].ParentIndex); writer.Write(meta[i].Value); }
                for (int i = 0; i < tectonic.Length; i++) { writer.Write(tectonic[i].IsOcean); writer.Write(tectonic[i].BaseHeight); }
                for (int i = 0; i < climate.Length; i++) { writer.Write(climate[i].Temperature); writer.Write(climate[i].Moisture); }
                for (int i = 0; i < hydro.Length; i++) {
                    writer.Write(hydro[i].FlowTargetIndex); writer.Write(hydro[i].Flux);
                    writer.Write(hydro[i].IsRiver); writer.Write(hydro[i].IsLake); writer.Write(hydro[i].IsOcean);
                }
                for (int i = 0; i < biomes.Length; i++) { writer.Write((int)biomes[i].Type); }

                // 2. ГЕОМЕТРИЯ
                writer.Write(geomVerts.Length);
                for(int i=0; i<geomVerts.Length; i++) { writer.Write(geomVerts[i].x); writer.Write(geomVerts[i].y); }
                ...
                {
                    var e = geomEdges[i];
                    writer.Write(e.SiteA); writer.Write(e.SiteB);
                    writer.Write(e.VertexA.x); writer.Write(e.VertexA.y);
                    writer.Write(e.VertexB.x); writer.Write(e.VertexB.y);
                }
            }
        }
    }
