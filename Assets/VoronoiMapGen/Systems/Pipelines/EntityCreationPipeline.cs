...
namespace VoronoiMapGen.Systems
{
    public static class EntityCreationPipeline
    {
        ...
        private static void TruncateAndCreateEntities(
            ...
            in NativeList<VoronoiEdge> edges)
        {
            int minCount = math.min(sites.Length, cells.Length);

            NativeArray<float2> truncatedSites = new NativeArray<float2>(minCount, Allocator.Temp);
            NativeArray<VoronoiSite> truncatedSiteMetadata = new NativeArray<VoronoiSite>(minCount, Allocator.Temp);
            NativeList<VoronoiCell> truncatedCells = new NativeList<VoronoiCell>(minCount, Allocator.Temp);
        ...
        private static void CreateLevelEntitiesInternal(
            ...
            in NativeList<VoronoiEdge> edges)
        {
            using NativeArray<Entity> siteEntities = CreateSiteEntities(em, level, levelSettings, sites, siteMetadata);
            using NativeArray<Entity> cellEntities = CreateCellEntities(em, level, levelSettings, cells);
        ...
        private static NativeArray<Entity> CreateSiteEntities(
            ...
            in NativeArray<VoronoiSite> siteMetadata)
        {
            NativeArray<Entity> siteEntities = new NativeArray<Entity>(sites.Length, Allocator.Temp);
        ...
        private static NativeArray<Entity> CreateCellEntities(
            ...
            in NativeList<VoronoiCell> cells)
        {
            NativeArray<Entity> cellEntities = new NativeArray<Entity>(cells.Length, Allocator.Temp);
            ...
            in NativeArray<Entity> cellEntities)
        {
            for (int i = 0; i < edges.Length; i++)
            {
                ...
                (Entity cellA, Entity cellB) = FindCellsForEdge(em, edge, cellEntities);
        ...
        private static (Entity, Entity) FindCellsForEdge(EntityManager em, VoronoiEdge edge, in NativeArray<Entity> cellEntities)
        {
            ...
            {
                Entity cEntity = cellEntities[j];
                VoronoiCell cell = em.GetComponentData<VoronoiCell>(cEntity);
